...
# Internal testing
root = '/beegfs/vle/Sabrina_Croce/GSMT_Survival/lightning_logs_Transformer_112024_fine_tuning/CONCH_finetune_STUMP_patients_IB_3CV_0_clinical_nofreeze/default/'

folds = ['fold_0', 'fold_1', 'fold_2']#, 'fold_3']#, 'fold_4']
ckpt_list = ['version_0/checkpoints/epoch=2-step=5.ckpt',
            'version_1/checkpoints/epoch=2-step=5.ckpt',
            'version_2/checkpoints/epoch=0-step=1.ckpt',
            #'version_3/checkpoints/epoch=9-step=19.ckpt',
            #'version_4/checkpoints/epoch=0-step=2.ckpt',
            ]
            
testing_patients = '/beegfs/vle/Sabrina_Croce/GSMT_Survival/exports_stumps_112024_fine_tuning/CONCH_finetune_STUMP_patients_IB_3CV_0_clinical_nofreeze'
root_folder = '/beegfs/vle/data/uterus_tumor_20X/CONCH_features'

#testing_patients = '/beegfs/vle/Sabrina_Croce/GSMT_data/v10_112024/rfs_STUMP_no_IB.csv'
#root_folder = '/beegfs/vle/data/uterus_tumor_20X_PTG/CONCH_features'
csv_save = 'exports_csv_v7_stumps_112024_fine_tuning/CONCH_finetune_STUMP_patients_IB_3CV_0_clinical_nofreeze'
nb_tile = 1000 # Get all tiles from Slide

##################### Train and valid inference ######################################################
for idx in range(len(folds)):
    fold_idx = folds[idx]
    ckpt_path = root + ckpt_list[idx]
    print(fold_idx, ckpt_list[idx])
    csv_save_idx = os.path.join(csv_save, 'Fold_'+str(idx))
    try:
        os.makedirs(csv_save_idx, exist_ok = True)
    except OSError:
        pass
    train_patient_csv = os.path.join(testing_patients,'Fold_'+str(idx), 'train.csv')
    train_cindex = load_patient_dict_and_inference(ckpt_path,
                                    root_folder, 
                                    train_patient_csv, 
                                    nb_tile,
                                    fold_idx,
                                    csv_save_idx)
    print("Training C-index: ", train_cindex)

    valid_patient_csv = os.path.join(testing_patients,'Fold_'+str(idx), 'valid.csv')
    #valid_patient_csv = testing_patients
    valid_cindex = load_patient_dict_and_inference(ckpt_path,
                                    root_folder, 
                                    valid_patient_csv, 
                                    nb_tile,
                                    fold_idx,
                                    csv_save_idx)
    print("Valid C-index: ", valid_cindex)
    
##################### If we have external test ######################################################
# root_folder = '/beegfs/vle/Sabrina_Croce/GSMT_data/STUMP_data/UNI_features'
# testing_patients_csv = '/beegfs/vle/Sabrina_Croce/GSMT_data/STUMP_data/csv_102024/cls_relapse_STUMP_with_clinical_IGR_only.csv'
# nb_tile = 5000 # Get all tiles from Slide
# for idx in range(len(folds)):
#     fold_idx = folds[idx]
#     ckpt_path = root + ckpt_list[idx]
#     print(fold_idx, ckpt_list[idx])
